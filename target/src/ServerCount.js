...
class ServerCount extends EventEmitter {
    ...
    constructor(client, options) {
        ...
        if (isNaN(options.Interval))
            throw new Error('The Interval must be only numbers');
        if (parseInt(options.Interval) < 1200000)
            throw new Error('The Interval must be higher than 20 minutes in ms');
        /**
         * The Discord Client
         * @type {Client}
         */
        ...
        setInterval(() => {
            try {
                fetch(`https://dsc.best/api/v3/${this.client.user.id}/stats`, {
                    ...
                    .then((data) => {
                    if (data.code !== 200)
                        return console.log(`Error: ${data.msg}`);
